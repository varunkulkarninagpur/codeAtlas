...
let graphData: { nodes: any[]; edges: any[]; violations: any[] } = { nodes: [], edges: [], violations: [] };
let selectedNodeId: string | null = null;
let activeHighlightMode: string | null = null;
...
const fitButton = document.getElementById("fit-btn") as HTMLButtonElement;
const relayoutButton = document.getElementById("relayout-btn") as HTMLButtonElement;
const focusButton = document.getElementById("focus-btn") as HTMLButtonElement;
const backButton = document.getElementById("back-btn") as HTMLButtonElement;
const forwardButton = document.getElementById("forward-btn") as HTMLButtonElement;
...
function getLayerIndex(type: string): number {
  switch (type) {
...
function calculateLayeredPositions(nodes: any[], width: number): Map<string, { x: number; y: number }> {
...
function getComponentAnnotation(type: string): string {
  switch (type) {
...
function initGraph() {
    ...
    style: [
      {
        ...
        style: {
          label: (ele: any) => {
            ...
            const fqName = ele.id();
            const nodeViolations = graphData.violations.filter((v) =>
              v.affectedNodes.includes(fqName)
            );
          ...
          "border-color": (ele: any) => {
            ...
            if (type === "CONTROLLER") return "#2196f3"; // Blue
            if (type === "SERVICE") return "#4caf50";    // Green
            if (type === "REPOSITORY") return "#ff9800"; // Orange
            if (type === "ENTITY") return "#9c27b0";     // Purple
  ...
  const checkBounds = () => {
    ...
    if (boundingBox.x1 === Infinity || isNaN(boundingBox.x1)) return;
    const margin = 100;
    ...
    if (x < minX) { x = minX; corrected = true; }
    if (x > maxX) { x = maxX; corrected = true; }
    if (y < minY) { y = minY; corrected = true; }
    if (y > maxY) { y = maxY; corrected = true; }
...
function selectNode(nodeId: string, pushHistory: boolean) {
  if (!cy) return;
  ...
  const comfortablyVisible =
    pos.x >= extent.x1 + xPad && pos.x <= extent.x2 - xPad &&
...
function navigateBack() {
  if (backStack.length === 0 || !selectedNodeId) return;
...
function navigateForward() {
  if (forwardStack.length === 0 || !selectedNodeId) return;
  backStack.push(selectedNodeId);
...
function applyPathHighlight(node: any, isSelection: boolean) {
  if (!cy) return;
...
function applyLayout(animate: boolean) {
  ...
  if (animate) {
    setTimeout(() => {
      cy?.animate({ fit: { padding: 30 } } as any, { duration: 250, easing: "ease-in-out-cubic" });
    }, 280);
  } else {
    cy.fit(undefined, 30);
  }
...
function getRuleRecommendation(type: string): string {
  switch (type) {
    case "BYPASS_SERVICE":
...
function updateInspector(metadata: any) {
  if (!metadata) return;
          ...
          <div style="margin-top: var(--spacing-xs); padding-top: var(--spacing-xs); border-top: 1px solid var(--vscode-panel-border, #3c3c3c); font-size: 10px; display: flex; flex-direction: column; gap: var(--spacing-xs);">
            ...
            <div><strong>Affected:</strong>
              <div class="chips-container" style="margin-top: 2px;">
                ${v.affectedNodes.map((nodeId: string) => {
                  const label = nodeId.split(".").pop();
                  return `<button class="clickable-violation-chip" data-id="${nodeId}">${label}</button>`;
                }).join("")}
              </div>
            </div>
            <div><strong>Dependency Chain:</strong> ${v.affectedNodes.map((nodeId: string) => nodeId.split(".").pop()).join(" &rarr; ")}</div>
  ...
  if (activeHighlightMode) {
    let friendlyLabel = "";
    if (activeHighlightMode === "incoming") friendlyLabel = "Incoming Dependencies";
    else if (activeHighlightMode === "outgoing") friendlyLabel = "Outgoing Dependencies";
    else if (activeHighlightMode === "entities") friendlyLabel = "Entity Mapping";
    else if (activeHighlightMode === "trace-chain") friendlyLabel = "Service Chain Trace";
    else if (activeHighlightMode === "violation-path") friendlyLabel = "Violation Path";
  ...
  if (activeHighlightMode) {
    ...
    document.getElementById("action-focus")?.addEventListener("click", () => {
      if (cy && node) {
        cy.animate({ center: node } as any, { duration: 200, easing: "ease-out" });
      }
    });
    ...
    document.getElementById("action-show-incoming")?.addEventListener("click", () => {
      ...
      cy.animate({ fit: { eles: related, padding: 80 } } as any, { duration: 250, easing: "ease-out" });
      updateInspector(metadata);
    });
    ...
    document.getElementById("action-show-entities")?.addEventListener("click", () => {
      if (!cy || !node) return;
      clearHighlight();
      const entities = node.outgoers("node").filter((n: any) => n.data("type") === "ENTITY");
    ...
    document.getElementById("action-trace-chain")?.addEventListener("click", () => {
      if (!cy || !node) return;
      clearHighlight();
      const services = node.outgoers("node").filter((n: any) => n.data("type") === "SERVICE");
      const repositories = services.outgoers("node").filter((n: any) => n.data("type") === "REPOSITORY");
    ...
    document.getElementById("action-highlight-violation")?.addEventListener("click", () => {
      ...
      cy.animate({ fit: { eles: violationPathEles, padding: 80 } } as any, { duration: 250, easing: "ease-out" });
