...
function parseExpectations(content, filePath) {
...
export async function runValidationSuite() {
    ...
    for (const javaClass of classes) {
        ...
        const nodeViolations = violations.filter((v) => v.affectedNodes.includes(javaClass.fullyQualifiedName));
        const hasExpectedViolation = nodeViolations.some((v) => v.type === expectedViolation);
        if (expectedViolation !== "none") {
            ...
            if (unexpectedViolations.length > 0) {
                failures.push(`✗ Rules Engine Mismatch in ${expectations.className}: Expected no violations, detected: ${unexpectedViolations.map((v) => v.type).join(", ")}`);
            }
        }
    }
