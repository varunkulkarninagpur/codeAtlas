 ...
 */
export class UnusedServiceRule {
    analyze(graph) {
        const violations = [];
        const components = graph.getAllComponents();
        for (const comp of components) {
            if (comp.type === "SERVICE") {
                const incoming = graph.getIncoming(comp.fullyQualifiedName, UnusedServiceRule.DEPENDENCY_ONLY).filter((sourceId) => sourceId !== comp.fullyQualifiedName);
                ...
                if (!isUsed) {
                    ...
                    for (const outId of outgoing) {
                        ...
                        if (targetComp) {
                            const targetIncoming = graph.getIncoming(outId, UnusedServiceRule.DEPENDENCY_ONLY).filter((srcId) => srcId !== comp.fullyQualifiedName && srcId !== outId);
...
UnusedServiceRule.DEPENDENCY_ONLY = { types: [RelationshipType.DEPENDENCY] };
UnusedServiceRule.DEPENDENCY_AND_INHERITANCE = {
