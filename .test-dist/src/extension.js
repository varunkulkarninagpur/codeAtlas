...
export function activate(context) {
    ...
    const openExplorerCommand = vscode.commands.registerCommand("codeatlas.openExplorer", async () => {
        ...
        if (AnalyzeProjectCommand.lastGraph) {
            ArchitectureExplorerPanel.createOrShow(context.extensionUri, AnalyzeProjectCommand.lastGraph, AnalyzeProjectCommand.lastViolations);
        }
        else {
            vscode.window.showErrorMessage("Failed to run analysis. Cannot open Architecture Explorer.");
        }
    });
    const selectNodeCommand = vscode.commands.registerCommand("codeatlas.selectNode", (nodeId) => {
