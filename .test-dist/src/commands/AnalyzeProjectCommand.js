...
export class AnalyzeProjectCommand {
    constructor(outputChannel, diagnosticPublisher, treeProvider) {
    ...
    async execute() {
        ...
        const verbose = vscode.workspace.getConfiguration("codeatlas").get("verboseLogging", false) ||
            vscode.workspace.getConfiguration("codeAtlas").get("verboseLogging", false);
        const logger = {
            info: (msg) => {
                if (verbose) {
...
AnalyzeProjectCommand.lastGraph = null;
