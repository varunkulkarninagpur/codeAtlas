...
describe("E2E Integration Verification with spring-demo", () => {
    ...
    it("should process spring-demo classes and flag exactly one direct controller-to-repository bypass violation", async () => {
        ...
        assert.ok(violation.message.includes("Controller 'BadController' directly depends on Repository 'UserRepository'"));
        assert.ok(violation.affectedNodes.includes("com.codeatlas.demo.controller.BadController"));
        assert.ok(violation.affectedNodes.includes("com.codeatlas.demo.repository.UserRepository"));
    });
